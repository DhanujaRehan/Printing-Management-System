...
async function loadPrintReport() {
  ...
  const kpiTotal  = document.getElementById('pr-kpi-total');
  const kpiBranch = document.getElementById('pr-kpi-branches');
  const kpiPrint  = document.getElementById('pr-kpi-printers');
  const kpiAvg    = document.getElementById('pr-kpi-avg');

  container.innerHTML = '<div class="loading"><div class="spin"></div>Loading print activity...</div>';

  const [logs, branches] = await Promise.all([
...
function renderPrintReport() {
  ...
  // Find max branch total for relative bar width
