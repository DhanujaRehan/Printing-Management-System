...
async function loadDashboard() {
  try {
    const [printers, stock, alerts, movements] = await Promise.all([
      api('GET', '/printers'),
      api('GET', '/toner/stock'),
      api('GET', '/toner/alerts'),
      api('GET', '/toner/movements?limit=8'),
    ]);

    // ── KPI Cards ──────────────────────────────────────────
    const total_stock      = stock.reduce((a, s) => a + s.quantity, 0);
    const low              = printers.filter(p => p.current_pct <= 25).length;
    const crit             = printers.filter(p => p.days_remaining <= 3 || p.current_pct <= 10).length;
    const branches_active  = [...new Set(printers.map(p => p.branch_code))].length;
    ...
    // ── Alert Strip ────────────────────────────────────────
    const crits = alerts.filter(p => p.current_pct <= 10 || p.days_remaining <= 3).slice(0, 2);
    const warns = alerts.filter(p => p.current_pct > 10 && p.current_pct <= 25 && p.days_remaining > 3).slice(0, 2);
      ...
      ...warns.map(p => `
        <div class="ac wn">
          ...
          <div>
            ...
            <div class="acsub">Branch ${p.branch_code} · ${p.days_remaining} days remaining · Next: ${p.next_toner_date ? new Date(p.next_toner_date).toLocaleDateString('en-GB',{day:'2-digit',month:'short'}) : '-'}</div>
          </div>
    ...
    // ── Printer Table ──────────────────────────────────────
    document.getElementById('pr-tbody').innerHTML = printers.map(p => `
      <tr>
    ...
    // ── Activity Feed ──────────────────────────────────────
    document.getElementById('dash-activity').innerHTML = movements.slice(0, 6).map((m, i) => `
      <div class="afi">
        <div class="afdc">
        ...
        <div style="flex:1">
          <div class="aft">${m.movement_type === 'IN'
            ? `Stock received — ${m.model_code} ×${m.quantity}`
            : `Toner installed — ${m.printer_code || '—'} (${m.model_code})`}</div>
